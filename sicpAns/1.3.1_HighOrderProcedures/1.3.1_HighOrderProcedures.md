...
(define (sum-pi n) (sum 1 n term-pi next-pi))
```
...
## Exercise 1.31 Product procedure for multiplication

### recursive procedure - solution a
```
...
### iterative procedure - solution b
```
...
## Exercise 1.32 General solution for both summarization and production

### Abstract procedure to Accumulate collections 
...
```
### Examples of using abstract **accumulate** procedure
```
...
- **iterative implementation**
```
    (define (accumulate combiner null-value term a b next filter)
      (define (iter-acc a result)
        (if(> a b)
    	  result
    	  (iter-acc (next a) (combiner (filter (term a)) result))))
      (iter-acc a null-value))
```
- **recursive implementation**
```
    (define (accumulate combiner null-value term a b next filter)
      (if(> a b)
        ...
        (if(filter a)		
          (combiner 
            (term a)
            (accumulate combiner null-value term (next a) b next filter))
          (accumulate combiner null-value term (next a) b next filter))))
```
### example of usage
...
```
    ...
    (define (sum-of-square-of-primes a b)
      ...
      (define (term x) (* x x))
      (define (next x) (+ x 1))
      (accumulate combiner null-value term a b next prime?))  
